...
```text
Constructor
  A special member used to initialize a new object.
  - Same name as the class
  - No return type
  - Called automatically when using `new`

Default constructor
  A no-argument constructor provided by the compiler only if you do not
  define any constructor yourself.

Parameterized constructor
  A constructor that accepts input parameters.
...
```text
Constructor
  Like the setup process when you buy a phone:
  language selection, fingerprint setup, accounts — it prepares the object for use.

Chaining
  Like one setup wizard step delegating to another step.
```
...
### Order of initialization (typical)

```text
new Child()
  |
  v
1) Parent() constructor runs first (via implicit/explicit super())
...
3) Therefore you cannot have both this(...) and super(...) in the same constructor
   because only one can be first.

Execution model:
  - When a child constructor runs, a parent constructor runs first.
...
```text
1) Defining ANY constructor removes the implicit default constructor.
   If you need a no-arg constructor, you must define it.

2) Calling this(...) not as the first statement -> compilation error.

3) Shadowing fields
   Use this.field to distinguish between parameter and field.

4) Validation
   Constructors should validate inputs to create objects in a valid state.
```
...
A) Only when you define no constructor at all.

Q) Difference between this(...) and super(...)?
A) this(...) calls another constructor in the same class.
   super(...) calls a constructor in the parent class.

Q) Why must this(...) be the first statement?
A) To guarantee a single, well-defined initialization path.
```
...
| Topic                 | Key point                                   |
+-----------------------+--------------------------------------------+
| Constructor           | Initializes object; no return type          |
| Default constructor   | only if you declared none                   |
| Overloading           | multiple constructors w/ different params   |
| Chaining              | this(...) or super(...), must be first line |
+-----------------------+--------------------------------------------+
```
