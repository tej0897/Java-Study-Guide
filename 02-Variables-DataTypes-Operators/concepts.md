...
Data type
  A rule that defines what kind of values a variable can hold.

Primitive type
  A built-in, fixed-size value type (int, long, double, boolean, char, ...).

Reference type (non-primitive)
  A variable that holds a reference to an object (String, arrays, classes, ...).
...
| Primitive              | Stores actual value, fixed-size           |
| Reference type         | Stores a reference to an object           |
| Operators              | Act on values (arithmetic/relational/...) |
| Common pitfalls        | overflow, casting, == vs =, precedence    |
+------------------------+------------------------------------------+
```
