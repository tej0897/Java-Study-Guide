...
```text
Method
  A named block of code that can be executed by calling it.

Parameter
  Input variable defined by the method.

Argument
  Actual value passed to the method when calling it.

Return value
  Output produced by a method (declared by the return type).

Signature
  Method name + parameter types (used for overload resolution).
```
...
```java
// A small, practical method with validation
...
```text
How a method call works (high level):

1) Caller evaluates arguments
2) JVM creates a new stack frame for the method
3) Parameters receive the argument values
4) Method body executes
5) Return value (if any) is passed back to caller
6) Stack frame is removed

Tip:
  If you can describe a repeated piece of logic in one sentence,
  it probably should be a method.
```
...
```text
1) Confusing parameters vs arguments
   parameter: (int x)
   argument : add(10)

2) Forgetting return statement for non-void methods

3) Writing huge methods
   If a method does too many things, split it (Single Responsibility).

4) Overloading confusion
   Overloading chooses method by parameter types, not return type.
```
...
1) What is method overloading?
   Same name, different parameter list (compile-time polymorphism).
...
| Good practice    | small methods + clear names + validation   |
+------------------+-------------------------------------------+
```
