...
```text
Class
  Blueprint that defines fields (state) and methods (behavior).

Object
  Runtime instance of a class, allocated in memory (heap).

State
  Data held by an object (fields).

Behavior
  Operations performed by an object (methods).

Constructor
  Special member used to initialize new objects.
...
```text
Class
  "Job description" for an employee.

Object
  One actual employee hired.

State vs Behavior
  State    -> employeeId, name, salary
  Behavior -> work(), requestLeave(), getPayslip()
```
...
```text
An object in memory (conceptual)

Reference variable (stack):
  accountRef  ----->  BankAccount object (heap)
...
```text
How to design a class (beginner-friendly checklist):

1) Identify responsibilities (what problem does it solve?)
2) Identify state (what does it need to remember?)
3) Identify behavior (what operations are needed?)
4) Decide visibility (private fields, public methods)
5) Validate inputs in public methods
6) Keep methods small and names clear
```
...
1) Making fields public
   Leads to "anyone can set any value" and breaks invariants.

2) God class
   A class that does everything. Use composition / split responsibilities.

3) Forgetting to override toString()
   Debugging becomes harder (you get ClassName@hashcode).

4) Confusing object identity vs equality
...
A) Class is definition; object is an instance created at runtime.

Q) Why encapsulate fields?
A) To protect invariants, add validation, and keep a stable API.
...
| Object            | runtime instance in heap                     |
| State             | fields/data                                  |
| Behavior          | methods                                     |
| Good design       | small + cohesive + validated + encapsulated  |
+-------------------+---------------------------------------------+
```
